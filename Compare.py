...
async def get_item_data(item_name: str, search_prompt: str):
    messages = [
        ...
        {"role": "user", "content": f"Search for and return information about: {item_name}"}
    ]
    ...
    try:
        ...
        return {"item": item_name, "error": f"Failed to parse item JSON: {e}", "raw": raw_text}
...
async def compile_comparison(item_results: list[dict], compare_prompt: str):
    messages = [
        ...
        {"role": "user", "content": f"Using the following independently retrieved item data, compile the final JSON comparison:\n\n{json.dumps(item_results)}"}
    ]
    ...
    try:
        ...
        return {"error": f"Error parsing final comparison: {e}", "raw": response.output_text}
